# GitHub Copilot Instructions - MkDocs Proof of Concept
...
### PART 2: MkDocs PoC Project Specific
- [Project Context](#project-context)
- [Professional Component Architecture](#professional-component-architecture-mandatory)
- [MkDocs-Specific Patterns](#mkdocs-specific-patterns)
- [Testing Patterns](#testing-patterns)
- [Zero-Setup Execution Pattern](#zero-setup-execution-pattern-mandatory)
- [File Naming Conventions](#file-naming-conventions-mandatory)
- [Common Tasks](#common-tasks)
- [Error Handling](#error-handling-mandatory)
...
- Best practices reference: `docs/eva-foundry/projects/07-foundation-layer/02-design/best-practices-reference.md`
- Marco Framework: `docs/eva-foundry/projects/07-foundation-layer/02-design/marco-framework-architecture.md`
...
This file is the copilot-instructions for **10-mkdocs-poc** (10-mkdocs-poc).
...
Once this file has been loaded, the active project is locked to **10-mkdocs-poc** for the entire session.
...
> **MkDocs Static Site Generation PoC**  
> Documentation infrastructure testing for SharePoint Online + Azure Static Website
...
**CRITICAL**: Read this ENTIRE file before generating ANY code for MkDocs PoC.
---
...
10-MkDocs-PoC/
 scripts/
    build_mkdocs.py              # Professional runner (main entry)
    test_hosting.py              # Hosting validation
    components/
        mkdocs_generator.py      # Site generator
        evidence_collector.py    # Debug artifacts
        validation_reporter.py   # Test reports
 mkdocs-sample/                   # Generated MkDocs site
    mkdocs.yml
    docs/                        # Source Markdown
    site/                        # Built HTML (generated)
 debug/                           # Debug artifacts (MANDATORY)
 evidence/                        # Test results (MANDATORY)
...
When creating components for this project:
...
class MkDocsComponent:
    """Base class for MkDocs PoC components"""
    ...
    def __init__(self, component_name: str):
        ...
        for path in [self.debug_path, self.evidence_path, self.logs_path, self.sessions_path]:
            path.mkdir(parents=True, exist_ok=True)
        
        self.timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    
    def execute_with_observability(self, operation_name: str, operation):
        """Execute operation with full evidence collection"""
        print(f"[INFO] Starting {operation_name}...")
        
        # Capture pre-state
        self.capture_debug_state(f"{operation_name}_before")
        
        try:
            result = operation()
            
            # Capture success state
            self.capture_debug_state(f"{operation_name}_success")
            print(f"[PASS] {operation_name} completed successfully")
            return result
            
        except Exception as e:
            # Capture error state
            self.capture_debug_state(f"{operation_name}_error")
            self.log_error(operation_name, e)
            print(f"[ERROR] {operation_name} failed: {str(e)}")
            raise
    
    def capture_debug_state(self, context: str):
...
```python
def generate_mkdocs_site(self, output_dir: Path):
    """Generate MkDocs site with EVA-style content"""
    
    # Create mkdocs.yml configuration
    mkdocs_config = {
        'site_name': 'EVA Docs Sample',
    ...
    # Write configuration
    config_file = output_dir / 'mkdocs.yml'
    # Implementation...
```

### Build Pattern
```python
def build_mkdocs_site(self, site_dir: Path):
    ...
    print(f"[PASS] MkDocs build successful")
    print(f"[INFO] Static site generated at: {site_dir / 'site'}")
```
...
### Hosting Validation Pattern
```python
def validate_hosting(self, platform: str, site_url: str):
    """Validate hosting platform capabilities"""
    ...
    test_result = self.test_page_renders(site_url)
    validation_results["tests"].append(test_result)
    
    # Test 2: CSS/JS assets load
    test_result = self.test_assets_load(site_url)
    validation_results["tests"].append(test_result)
    
    # Test 3: Internal navigation works
    test_result = self.test_internal_navigation(site_url)
    validation_results["tests"].append(test_result)
    
    # Test 4: Deep links work
    test_result = self.test_deep_links(site_url)
    validation_results["tests"].append(test_result)
    
    # Save results
    results_file = self.evidence_path / "test-results" / f"{platform}_validation_{self.timestamp}.json"
...
## Zero-Setup Execution Pattern (MANDATORY)

The main runner must implement:

```python
class ProfessionalMkDocsRunner:
    """Professional runner for MkDocs PoC"""
    ...
    def validate_environment_pre_flight(self) -> bool:
        ...
        try:
            import mkdocs
            print(f"[PASS] MkDocs {mkdocs.__version__} found")
        except ImportError:
            print("[WARN] MkDocs not found, installing...")
            # Install from requirements
        
        # Check required directories
        required_dirs = ['scripts', 'debug', 'evidence', 'logs']
        for dir_name in required_dirs:
            dir_path = self.project_root / dir_name
            if not dir_path.exists():
...
All generated files must use timestamped naming:

| Artifact Type | Pattern | Example |
|---------------|---------|---------|
| Debug State | `{component}_debug_{context}_{timestamp}.json` | `mkdocs_debug_build_success_20260124_143022.json` |
| Test Results | `{platform}_validation_{timestamp}.json` | `spo_validation_20260124_143530.json` |
| Execution Logs | `{component}_execution_{date}.log` | `build_execution_20260124.log` |
| Evidence Reports | `{component}_evidence_{timestamp}.json` | `hosting_evidence_20260124_144012.json` |
...
### Run Full PoC (generate + build + validate)
```python
...
```python
python scripts/test_hosting.py --platform spo --url https://your-spo-site.com
python scripts/test_hosting.py --platform azure --url https://your-static-site.azure.com
```
...
All errors must:
1. Use ASCII-only output (`[ERROR]` not [x])
2. Capture debug state at error point
3. Log structured error to logs/
...
try:
    ...
    self.capture_debug_state("build_error")
    self.log_error("build_site", e)
...
- [x] All output uses ASCII-only characters
- [x] All operations capture debug state (before/success/error)
- [x] All errors logged with full context
- [x] All artifacts use timestamped naming
- [x] All directories created if missing
- [x] All file I/O uses UTF-8 encoding explicitly
- [x] Professional runner auto-detects project root
- [x] Pre-flight validation before execution

---

## References

- EVA Professional Component Architecture (Project 06)
- EVA Copilot Standards (Project 07)
